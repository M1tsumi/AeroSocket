...
impl TlsTransport {
    ...
    pub fn with_config(config: ServerConfig, addr: SocketAddr) -> impl std::future::Future<Output = Result<Self>> + Send {
        Box::pin(async move {
            let listener = tokio::net::TcpListener::bind(addr)
                .await
                .map_err(aerosocket_core::Error::Io)?;
                    ...
                    match &self.acceptor {
                        Some(acceptor) => {
                            ...
                            let tls_stream = acceptor.accept(stream)
                                .await
                                .map_err(|e| aerosocket_core::Error::Other(format!("TLS handshake failed: {}", e)))?;
                            
                            Ok(TlsStream::from_server_tls_stream(tls_stream))
                        }
                        None => Err(aerosocket_core::Error::Other("TLS acceptor not configured".to_string())),
                    }
                }
...
impl TransportStream for TlsStream {
    async fn read(&mut self, buf: &mut [u8]) -> Result<usize> {
        match &mut self.stream {
            ...
            None => Err(aerosocket_core::Error::Other("TLS stream not connected".to_string())),
        }
    }
